...
detail.
-}
type alias Html msg = Signal.Address msg -> VirtualDom.Node


{-| Set attributes on your `Html`.
-}
type alias Attribute msg = Signal.Address msg -> VirtualDom.Property


{-| Map child's actions to the child
-}
map : (msg -> parent) -> Html msg -> Html parent
map generalize html addr =
  html (Signal.forwardTo addr generalize)


{-| Forward arbitrary actions to the child
-}
forward : Signal.Address msg -> Html msg -> Html parent
forward addr html =
  \_ -> html addr
...
node tag attributes children addr =
  VirtualDom.node
    tag
    (List.map (\a-> a addr) attributes)
    (List.map (\a-> a addr) children)
...
-}
toElement : Int -> Int -> Html msg -> Signal.Address msg -> Element
toElement w h html addr=
    VirtualDom.toElement w h (html addr)
...
component.
-}
fromElement : Element -> Html a
fromElement el =
  \_ -> VirtualDom.fromElement el
...
{-| Embedded an Html msg document. -}
iframe : List (Attribute msg) -> List (Html msg) -> Html msg
iframe =
  node "iframe"


{-| Represents a integration point for an external, often non-Html msg,
...
{-| Represents an external resource, which is treated as an image, an Html msg
